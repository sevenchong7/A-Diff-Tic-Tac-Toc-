...
const skillLockCard = player1.find(
  (card) => card.type === "OPPONENT_SKILL_LOCK"
);

if (!skillLockCard) {
  throw new Error(
    "Skill Lock card was not found in Player 1's hand"
  );
}
...
const player2Card = player2Hand.find(
  (card) => card.type !== "OPPONENT_SKILL_LOCK"
);
...
try {
  ...
  throw new Error(
    "Expected Player 2 to be blocked from using a Skill Card"
  );
} catch (error) {
  console.log(
    "Expected error:",
...
const player2CardAfterLock =
  game.getPlayerHand("player2").find(
    (card) => card.type !== "OPPONENT_SKILL_LOCK"
  );

if (!player2CardAfterLock) {
  throw new Error(
    "Player 2 does not have a suitable Skill Card"
  );
}
...
try {
  game.useCard(
    "player2",
    player2CardAfterLock.id,
    {}
  );

  console.log(
    "Player 2 can use Skill Cards again"
  );
} catch (error) {
  console.log(
    "Unexpected error:",
