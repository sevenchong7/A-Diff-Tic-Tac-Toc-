...
const card = game
    ...
    .find((card) => card.type === "REMOVE_ROW_COLUMN");
if (!card) {
    throw new Error("Remove Row / Column card was not found");
}
...
console.log(game.getPlayerHand("player1"));
console.log("Board after removing row:", game.getBoard().getData());
...
const secondCard = game
    ...
    .find((card) => card.type === "REMOVE_ROW_COLUMN");
if (!secondCard) {
    throw new Error("Second Remove Row / Column card was not found");
}
game.useCard("player1", secondCard.id, {
    lineType: "column",
    position: 1,
});
console.log("Board after removing column:", game.getBoard().getData());
